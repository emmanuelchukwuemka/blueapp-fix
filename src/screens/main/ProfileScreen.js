    ...
    const MenuItem = ({ icon, title, subtitle, onPress, showChevron = true, isDestructive = false }) => (
        <TouchableOpacity onPress={onPress}>
            <View style={styles.menuItem}>
                <View style={[styles.menuIcon, { backgroundColor: isDestructive ? '#FFEBEE' : (icon === 'account-multiple' ? '#E8F5E9' : '#E3F2FD') }]}>
                ...
                <Surface style={styles.profileCard} elevation={3}>
                    <View style={styles.avatarRow}>
                        <View style={styles.avatarContainer}>
                            {user.profileImage ? (
                                ...
                                <Avatar.Text size={80} label={user.full_name || user.name ? (user.full_name || user.name).substring(0, 2).toUpperCase() : 'BP'} style={{ backgroundColor: Colors.primary }} />
                            )}
                        ...
                        <View style={styles.userInfo}>
                            <Text variant="headlineSmall" style={styles.userName}>{user.full_name || user.name}</Text>
                    ...
                    <View style={styles.statsRow}>
                        <View style={styles.statItem}>
                            <Text style={styles.statValue}>{user.points_balance?.toLocaleString() || '0'}</Text>
                            <Text style={styles.statLabel}>Points</Text>
                        </View>
                        <View style={styles.verticalDivider} />
                        <View style={styles.statItem}>
                            <Text style={styles.statValue}>{user.total_points_earned?.toLocaleString() || '0'}</Text>
                            <Text style={styles.statLabel}>Earned</Text>
                        </View>
                        <View style={styles.verticalDivider} />
                        <View style={styles.statItem}>
                            <Text style={styles.statValue}>{user.total_points_withdrawn?.toLocaleString() || '0'}</Text>
            ...
            <View style={styles.sectionContainer}>
                ...
                <Surface style={styles.menuCard} elevation={1}>
                    ...
                    <MenuItem
                        ...
                        showChevron={false}
                    />
                    {/* Hidden debug menu item - only shown in development */}
                    {__DEV__ && (
                        <MenuItem
                            icon="bug"
                            title="API Debug"
